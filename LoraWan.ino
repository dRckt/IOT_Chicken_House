...
void do_send(osjob_t* j){
    ...
    if (LMIC.opmode & OP_TXRXPEND) {
      ...
      // encode int as bytes
      //byte payload[2];
      loraData[0] = ChickenNbr; //highByte(tempInt);  // left byte, lower value
      
      loraData[1] = light; //lowByte(tempInt);
      
      loraData[2] = DoorState;//highByte(humidInt);
      
      //loraData[3] = Door; //lowByte(humidInt);
